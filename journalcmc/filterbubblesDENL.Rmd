...
# Introduction
The internet has lifted the restriction on the amount of media we are able to acquire by making the production of media easier, cheaper and more dynamic. In the discussion on whether this is a positive change, politics seems to be a recurring theme [e.g., @garrett2009; @colleoni2014]. Due to the fear that the large amount of available media might result in users mainly consuming views they already agree with, the threat of polarization, fragmentation, segregation, and even extremism has been expressed. When discussing these possible undesirable consequences of the internet, the term *filter bubble* is often mentioned. This term coined by Pariser [-@pariser2011] describes how personalized filtering algorithms put people in "unique universes of information" in which different people receive different truths. The users are thereby not necessarily aware that the content they perceive is being filtered. Although this unawareness has often been suggested [e.g., @garrett2009; @colleoni2014], only few have actually tested this assumption [e.g., @epstein2015; @eslami2015]. Furthermore we are not aware of any studies specifically testing what user aspects cause people to be (un)aware of filter bubbles and whether this is influenced by nationality. Since knowledge on this topic can be used to increase the awareness of filter bubbles and prevent the phenomenon from negatively influencing democracies, the present work will answer the following question:  

> *RQ*: How does user diversity influence the awareness of filter bubbles regarding political opinion formation in the Netherlands and Germany?  


# Related Work
Although the internet provides a lot of information on politics and an accessible place for political discussion [@brundidge2010; @messing2014], it might be found to limit the different opinions people encounter and mainly promote discussion between like-minded individuals. This then endangers our (deliberative) democracies which rely on political conversation between people who are well-informed on the different political opinions. As Sunstein [-@sunstein2017, 63--64] puts it: "There is an omnipresent risk of too many options, too many topics, and too many opinions." Since it is not possible to consume everything, choices have to be made.  

As explained by Festinger's [-@festinger1957] cognitive dissonance theory, consumers prefer media that coincides with their already existing beliefs [e.g., @beam2014; @dubois2018]. This is said to be reflected in the media consumers decide to use, resulting in echo chambers [@sunstein2017, 1--12 & 63--68]. However, it is also taken into account by the filtering algorithms that have been developd to help us deal with the abundance of available information on the Internet [@pariser2011, 10--12]. The reason for this is that the succes of a filtering algorithm is measured in consumer satisfaction, which is likely to increase if the content corresponds to what the consumer is interested in or looking for. Since users differ among each other, personalized filtering algorithms are often used to reach this goal [e.g., @koene2015]. 


## Filtering alogrithms
Filtering algorithms have been developed to help us deal with the abundance of available information on the Internet [@pariser2011, 10--12]. At this moment, the scope of the websites that make use of such filtering algorithms is broad, but well-known examples are Google's search engine and the social networking site Facebook [e.g., @hannak2013; @bakshy2015]. The success of a filtering algorithm is measured in consumer satisfaction. Since the amount of media is no problem, it is likely that consumer satisfaction is determined by how well the content that is shown corresponds to what a consumer is interested in or looking for. Filtering algorithms are therefore often personalized . There are many different personalization techniques. Frequently used examples are content-based personalizing, which uses the media uploaded by users to figure out what they like to see, and collaborative filtering which uses the preferences of people who show similar interests to make predictions about what a person might like [@bellogin2013; @koene2015]. Moreover, multiple techniques are often combined, and researchers have even started experimenting with using machine learning in personalization algorithms [@portugal2017]. All in all, developers are striving to make personalization algorithms better and more accurate.  
